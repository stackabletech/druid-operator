#!/usr/bin/env bash
# This script reads a Helm chart from deploy/helm/druid-operator and
...
helm template --output-dir "$tmp" \
              --include-crds \
              --name-template druid-operator \
              deploy/helm/druid-operator

for file in $tmp/druid-operator/*/*; do
...
cp -r "$tmp"/druid-operator/*/* deploy/manifests/
