...
async fn main() -> anyhow::Result<()> {
    ...
    match opts.cmd {
        ...
        Command::Run(RunArguments {
            ...
            let druid_cluster_check = readiness_checks.register(format!(
                "CRD {crd} installed",
                crd = v1alpha1::DruidCluster::crd_name()
            ));
            ...
            let delayed_druid_controller = async {
                signal::crd_established(&client, v1alpha1::DruidCluster::crd_name()).await?;
                druid_cluster_check.mark_passed();
