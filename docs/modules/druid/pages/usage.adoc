...
Using an LDAP server **without** bind credentials is not supported. This limitation is due to Druid not supporting this scenario. See [our issue](https://github.com/stackabletech/druid-operator/issues/383) for details.
...
=== Adding External Files to MiddleManagers for Ingestion
...
In order to make these files available the operator allows specifying extra volumes that will be added to the MiddleManager pods deployed for this cluster.
