...
spec:
  clusterConfig:
    tls:
      secretClass: tls # <1>
----
<1> Name of the `SecretClass` that is used to encrypt communication.
...
The access to the Druid cluster can be limited by configuring client authentication (mutual TLS) for all participants. This means that clients (e.g. the Browser or a Broker) have to authenticate themselves with valid certificates in order to communicate with the Druid cluster.
