...
The Stackable Platform uses a common set of resource definitions for s3 across all operators, explained in detail on the link:TODO[S3 resources] page. In general, you can configure an S3 connection or bucket inline, or as a reference to a dedicated object.
...
You can specify just a connection/bucket or just one of these or for both, but Druid only supports a single S3 endpoint under the hood, so if two connections are specified, they must be the same. This is easiest if a dedicated link:to_s3_docs[S3 Connection Resource] is used.
