...
To use LDAP, you will need to specify a xref:commons-operator::authenticationclass.adoc[`AuthenticationClass`] which is used to authenticate the users.
Here is a reduced, non-functional YAML example, which shows the most essential parts which are needed to configure LDAP in the druid-operator:
...
Authorization is done using the `allowAll` authorizer. In the future, support for `memberOf` and OPA will be added.
