...
Historical Pods store the segment cache on `emptyDir` node volumes. You can configure the maximum size of this cache with `segmentCacheSize`. The operator will then create volumes by adding 5% free space to specified cache size. The default value of the segment cache is `1g` and a volume size of `1.05g`.
