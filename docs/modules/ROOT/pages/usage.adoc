...
In this example, each Druid broker in the `default` group will have a maximum of `3Gi` (gigabytes) of memory. To be more precise, these memory limits apply to the container running Druid but not to any sidecar containers that are part of the pod.
