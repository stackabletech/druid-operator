...
The cluster definition also supports overriding configuration properties, either per role or per role group, where the more specific override (role group) has precedence over the less specific one (role).
...
// envOverrides are not implemented and also are not necessary since druid doesn't read env vars
