...
After going through the xref:installation.adoc[] section and having installed all the Operators, you can deploy product instances. You will also install an HDFS instance which is used as a deep storage backend by Druid, as well as a ZooKeeper instance which is needed by both Druid and HDFS.
...
To open the user interface navigate your browser to https://localhost:8888/ to find the dashboard:
...
Have a look at the xref:ROOT:usage.adoc[] page to find out more about features of the Operator, such as S3 backed deep storage or OPA based authorization.