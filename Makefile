...
VERSION := $(shell cargo metadata --format-version 1 | jq '.packages[] | select(.name=="stackable-druid-operator") | .version')
...
version:
	yq eval -i '.version = ${VERSION} | .appVersion = ${VERSION}' /dev/stdin < deploy/helm/druid-operator/Chart.yaml
